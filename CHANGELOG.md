...
Каждый эндпоинт в таблице «Локальные RPC-ноды» теперь показывает бейджи
активных протоколов рядом с именем: `pokersth [rpc] [ws]`.
- `[rpc]` - всегда (HTTP `local_url`)
- `[ws]` - только если задан `local_ws_url` в config.yaml

Это позволяет одним взглядом понять, готов ли провайдер обслуживать
WebSocket-туннели (например, для Socket.IO DApp'ов). JSON `/status` теперь
возвращает `rpc[].protocols: string[]`.

### Added - WS-туннель: диагностические логи

`handle_ws_stream` теперь пишет `tracing::info!` на каждом ключевом шаге:
- `WS[<provider>] handshake от peer=<hex> path=<path>` - входящий WS-запрос
- `WS[<provider>] апстрим: <ws-url>` - резолвленный `local_ws_url` + путь
- `WS[<provider>] апстрим OK (HTTP 101)` - upstream Socket.IO/WS ответил
- `WS[<provider>] upstream connect FAILED: …` - не удалось подключиться к
  локальному WS-серверу (порт закрыт, wrong host, TLS mismatch)
- `WS[<provider>] upstream connect TIMEOUT (10s)` - локальный WS повис
- `WS[<provider>] отклонён: local_ws_url не задан в config.yaml`

Без этих логов было невозможно отличить «WS-запрос вообще не пришёл» от
«пришёл, но upstream упал». Теперь любой сбой WS-туннеля виден в `INFO`.

### Improved - понятное сообщение об ошибке в close(1011)

Reason-строка теперь указывает точное расположение недостающего поля в
